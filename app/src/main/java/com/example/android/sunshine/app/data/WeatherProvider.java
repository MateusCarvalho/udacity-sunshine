...
public class WeatherProvider extends ContentProvider {
    ...
    public int update(
        ...
        switch (match) {
            case WEATHER: {
                rowsAffected = database.update(WeatherContract.WeatherEntry.TABLE_NAME,values,selection,selectionArgs);
                if (rowsAffected==0) {
            ...
            case LOCATION: {
                rowsAffected = database.update(WeatherContract.LocationEntry.TABLE_NAME,values,selection,selectionArgs);
                if (rowsAffected==0) {
                    throw new SQLException("Failed to update row into" + uri);
                }
                break;
